...
// Phase 1c: MCP Tools Integration
package main
...
func main() {
	...
	case "serve": cmdServe()
	case "mcp": cmdMCP()
	case "version": fmt.Println("nyawa v0.3.0 — Phase 1c")
	default:
...
func cmdRecall() {
	...
	s := getStore(os.Args[2]); defer s.Close()
	p := search.NewPipeline(s, getEmbedder(), types.DefaultConfig().Search)
