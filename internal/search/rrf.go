...
func (r *RRF) Fuse(vectorIDs, fts5IDs []string) []FusionResult {
	...
	for rank, id := range vectorIDs {
		...
		} else { seen[id].VectorRank = rank + 1 }
	}
...
func NewPostProcessor(recencyWeight, importanceWeight float64, rp *pool.ResultPool) *PostProcessor {
	if recencyWeight <= 0 { recencyWeight = 0.05 }
	if importanceWeight <= 0 { importanceWeight = 0.10 }
	return &PostProcessor{RecencyWeight: recencyWeight, ImportanceWeight: importanceWeight, resultPool: rp}
}
