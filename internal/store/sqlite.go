...
func NewStore(dbPath string, emb Embedder) (*Store, error) {
	...
	s := &Store{db: db, hnsw: index.NewHNSW(index.DefaultHNSWConfig(dim)), embedder: emb}
...
func (s *Store) InsertMemory(m *types.Memory) error {
	...
	if err != nil { return fmt.Errorf("insert memory: %w", err) }
	if s.embedder != nil && s.embedder.Available() {
		vec, err := s.embedder.Embed(m.Content)
		if err == nil && len(vec) > 0 { s.hnsw.Insert(m.ID, vec) }
	}
	return nil
}
...
func (s *Store) VectorSearch(queryVector []float32, topK int, namespace string) ([]string, error) {
	if queryVector == nil || len(queryVector) == 0 { return nil, nil }
	results := s.hnsw.Search(queryVector, topK*3)
	if len(results) == 0 { return nil, nil }
	ids := make([]string, 0, len(results))
	for _, r := range results { ids = append(ids, r.ID) }
	return ids, nil
}
...
func (s *Store) FTS5Search(query string, topK int, namespace string) ([]string, error) {
	...
	if err != nil { return nil, fmt.Errorf("fts5: %w", err) }
	defer rows.Close()
	var ids []string
