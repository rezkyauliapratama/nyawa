...
func (s *Store) migrate() error {
	...
	CREATE TRIGGER IF NOT EXISTS memories_ad AFTER DELETE ON memories BEGIN
		INSERT INTO memories_fts(fts_idx, rowid, content) VALUES('delete', old.rowid, old.content);
	END;
...
func (s *Store) GetMemoriesByIDs(ids []string) ([]*types.Memory, error) {
	...
	query := fmt.Sprintf(`SELECT id, content, mem_type, namespace, importance, access_count, pinned, created_at, updated_at, superseded_at, edge_count FROM memories WHERE id IN (%s)`, joinString(placeholders, ","))
...
func (s *Store) Stats() (map[string]any, error) {
	...
	s.db.QueryRow(`SELECT COUNT(*) FROM memories_fts`).Scan(&ftsSize)
	return map[string]any{"total_memories": total, "pinned_memories": pinned, "fts_indexed": ftsSize}, nil
}
