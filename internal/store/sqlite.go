...
func (s *Store) InsertMemory(m *types.Memory) error {
	...
	if s.graph != nil && s.classify != nil {
		...
		if n, err := s.graph.InsertMemoryEntities(m.ID, entities); err == nil && n > 0 {
	...
	if s.embedder != nil && s.embedder.Available() {
		if vec, err := s.embedder.Embed(m.Content); err == nil && len(vec) > 0 {
			s.hnsw.Insert(m.ID, vec); s.persistHNSW()
		}
	}
	return nil
}
...
func (s *Store) GetMemory(id string) (*types.Memory, error) {
	...
	if err := s.db.QueryRow(`SELECT id,content,mem_type,namespace,importance,access_count,pinned,created_at,updated_at,superseded_at,edge_count FROM memories WHERE id=?`, id).Scan(&m.ID, &m.Content, &mt, &m.Namespace, &m.Importance, &m.AccessCount, &pi, &cs, &us, &ss, &m.EdgeCount); err != nil { return nil, err }
	m.Type = types.MemoryType(mt); m.Pinned = pi != 0; m.CreatedAt, _ = parseTime(cs); m.UpdatedAt, _ = parseTime(us)
...
func (s *Store) GetEntityStats() (map[string]any, error) {
	if s.graph == nil { return map[string]any{"en": 0, "ee": 0}, nil }
	return s.graph.Stats()
}

func (s *Store) IncrementAccessCount(id string) error {
	_, err := s.db.Exec(`UPDATE memories SET access_count=access_count+1 WHERE id=?`, id); return err
}

func (s *Store) VectorSearch(q []float32, k int, ns string) ([]string, error) {
	if len(q) == 0 { return nil, nil }
	r := s.hnsw.Search(q, k*3)
...
func (s *Store) Stats() (map[string]any, error) {
	...
	en, ee := 0, 0
	if s.graph != nil { if st, err := s.graph.Stats(); err == nil { en = st["entity_nodes"].(int); ee = st["entity_edges"].(int) } }
